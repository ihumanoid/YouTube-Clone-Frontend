...
function EVideoPlayer({ youtubeId, updateVideoData }: EVideoPlayerProps) {
  ...
  const handleProgress = (progress: any) => {
    ...
    // if (progress.playedSeconds - prevPlayedSeconds >= 1) {
    //   console.log(
    //     `Skipped ${Math.floor(
    //       progress.playedSeconds - prevPlayedSeconds
    //     )} prev = ${prevPlayedSeconds}`
    //   );
    //   reactPlayerRef.current?.seekTo(Math.floor(prevPlayedSeconds));
    //   return;
    // }
  ...
  const getHideElements = () => {
    return (
      <div>
        ...
        <div className="absolute w-full h-[88px] top-0 right-0 transparent bg-black flex justify-end">
